...
```{r}
boxplot(tail ~ gender, data = new.dat, 
        main = "Tail Measurements")
```

##

```{r}
boxplot(wing ~ gender, data = new.dat, 
        main = "Wing Measurements")
```
...
## Post-Hoc Tests, Tail Measurements Only

```{r}
t.test(tail ~ gender, data = new.dat)
```
Strong evidence that female birds have longer average tail lengths than males.


## Post-Hoc Tests, Wing Measurements Only

```{r}
t.test(wing ~ gender, data = new.dat)
```
Not enough evidence that female and male birds have different wing measurements.
